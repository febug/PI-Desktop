...
test("remote hosts is an inventory plus one SSH/Pair add form", () => {
  ...
  for (const mode of ["ssh", "pair"]) {
    assert.match(page, new RegExp(`id: "remote-host-add-${mode}", controls: "remote-host-add-panel-${mode}"`));
    assert.match(page, new RegExp(`aria-labelledby="remote-host-add-${mode}"`));
  }
